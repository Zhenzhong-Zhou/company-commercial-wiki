---
id: TERM-TABLET
title_en: Tablet
title_zh: 片剂
page_type: term
status: current
canonical_language: en
supported_languages: [en, zh]
updated_at: 2026-04-22
review_required: false
native_review_required: false
source_ids: [SRC-WIDE-DECK-EN, SRC-WIDE-BONE-CN-CARD]
related_pages:
  - "[[SRC-WIDE-DECK-EN]]"
  - "[[SRC-WIDE-BONE-CN-CARD]]"
  - "[[PROD-D3-K2-CA-MG-ZN]]"
tags: [term, dosage-form, material, bilingual]
---

# Tablet

## 中文名称
片剂

## Canonical English Term
Tablet

## Preferred Chinese Translation
片剂

## Aliases
- Tablet form
- 干压片剂
- 片剂形态

## Discouraged Variants
- Softgel, when the source says tablet.
- Capsule, when the source says tablet.

## Domain
- material
- product facts
- translation

## Definition (EN)
Tablet is a dosage-form term used for compressed or tablet-form products. In this wiki, it should describe form only; stability or efficacy claims attached to tablet form require separate evidence.

## 中文解释
片剂用于描述压片或片状剂型。手卡中的干压片剂稳定性表达属于营销或材料主张，需要额外证据支持。

## Usage Context
- product identity
- dosage form
- material terminology
- translation

## Approved Usage Examples
### English
- "Dosage form: tablet."

### 中文
- "剂型：片剂。"

## Discouraged Usage Examples
### English
- "Tablet form proves nutrients stay fresh."

### 中文
- "片剂一定能锁住鲜活营养。"

## Notes
- literal translation: 片剂
- business rewrite: tablet dosage form
- status note: Current for terminology normalization only.
- review behavior: Human review is not required for the basic term mapping; product-specific form claims and stability or efficacy claims remain source-linked and review-sensitive.
- risk notes: Low terminology risk; stability claims remain review-sensitive.

## Source Map
- [[SRC-WIDE-DECK-EN]]: WIDE NATURALS DECK-ENpptx.pptx
- [[SRC-WIDE-BONE-CN-CARD]]: WideNaturals加拿大五合一维骨力头部手卡.pptx
