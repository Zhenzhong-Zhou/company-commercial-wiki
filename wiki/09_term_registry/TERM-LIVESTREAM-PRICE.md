---
id: TERM-LIVESTREAM-PRICE
title_en: Livestream Price
title_zh: 直播价
page_type: term
status: pending_confirmation
canonical_language: en
supported_languages: [en, zh]
updated_at: 2026-04-22
review_required: true
native_review_required: true
source_ids: [SRC-PG-ASTAXANTHIN-CN-CARD, SRC-PG-UBIQUINOL-CN-CARD, SRC-WIDE-BONE-CN-CARD]
related_pages:
  - "[[SRC-PG-ASTAXANTHIN-CN-CARD]]"
  - "[[SRC-PG-UBIQUINOL-CN-CARD]]"
  - "[[SRC-WIDE-BONE-CN-CARD]]"
  - "[[PROD-ASTAXANTHIN-PLUS]]"
  - "[[PROD-UBIQUINOL]]"
  - "[[PROD-D3-K2-CA-MG-ZN]]"
  - "[[PRICE-D3-K2-CA-MG-ZN]]"
tags: [term, pricing, ecommerce, livestream, bilingual, native-review]
---

# Livestream Price

## 中文名称
直播价

## Canonical English Term
Livestream Price

## Preferred Chinese Translation
直播价

## Aliases
- Live-commerce price
- Live price
- 直播活动价

## Discouraged Variants
- Official price, unless a formal pricing policy says so.
- Current price, when the date and campaign context are not verified.
- Retail price, when the source specifically labels it as livestream price.

## Domain
- pricing
- inquiry handling
- ecommerce
- customer-facing review

## Definition (EN)
Livestream Price is a channel- and campaign-sensitive price shown in sales-card evidence. It must not be treated as a timeless or official quote without current channel verification.

## 中文解释
直播价属于渠道和活动敏感价格。来源中出现直播价时，只能作为价格证据保存，不能自动作为当前报价或长期有效价格。

## Usage Context
- pricing evidence
- livestream sales material
- customer inquiry handling
- quotation review

## Approved Usage Examples
### English
- "The source lists a livestream price, but the date and campaign context are not stated."

### 中文
- "来源列出直播价，但未说明日期和活动周期，不能直接报价。"

## Discouraged Usage Examples
### English
- "The livestream price is the current official price."

### 中文
- "直播价就是当前官方价。"

## Notes
- literal translation: 直播价
- business rewrite: live-commerce campaign price
- risk notes: Pricing is high risk and requires fresh verification.

## Graph Links
- Source notes with livestream-price evidence: [[SRC-PG-ASTAXANTHIN-CN-CARD]], [[SRC-PG-UBIQUINOL-CN-CARD]], [[SRC-WIDE-BONE-CN-CARD]]
- Product pages connected through source evidence: [[PROD-ASTAXANTHIN-PLUS]], [[PROD-UBIQUINOL]], [[PROD-D3-K2-CA-MG-ZN]]
- Normalized pricing page: [[PRICE-D3-K2-CA-MG-ZN]]

## Source Map
- [[SRC-PG-ASTAXANTHIN-CN-CARD]]: PG加拿大品牌天然复合虾青素（plus）头部手卡.pptx
- [[SRC-PG-UBIQUINOL-CN-CARD]]: PG加拿大品牌第四代共晶还原性辅酶Q10头部手卡.pptx
- [[SRC-WIDE-BONE-CN-CARD]]: WideNaturals加拿大五合一维骨力头部手卡.pptx
- [[PRICE-D3-K2-CA-MG-ZN]]
