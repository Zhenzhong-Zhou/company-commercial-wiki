---
id: TERM-SOFTGEL
title_en: Softgel
title_zh: 软胶囊
page_type: term
status: current
canonical_language: en
supported_languages: [en, zh]
updated_at: 2026-04-22
review_required: false
native_review_required: false
source_ids: [SRC-WIDE-DECK-EN, SRC-PG-UBIQUINOL-CN-CARD]
related_pages:
  - "[[SRC-WIDE-DECK-EN]]"
  - "[[SRC-PG-UBIQUINOL-CN-CARD]]"
  - "[[PROD-UBIQUINOL]]"
tags: [term, dosage-form, material, bilingual]
---

# Softgel

## 中文名称
软胶囊

## Canonical English Term
Softgel

## Preferred Chinese Translation
软胶囊

## Aliases
- Soft gel
- Soft capsule
- 软胶囊剂型

## Discouraged Variants
- Capsule, when the source specifically says softgel.
- Tablet, when describing a softgel product.

## Domain
- material
- product facts
- translation

## Definition (EN)
Softgel is a dosage-form term used for soft capsule products. In this wiki, it should be recorded as a product form only, not as evidence of superior absorption unless the source and claim are separately validated.

## 中文解释
软胶囊是剂型术语，用于描述软质胶囊类产品。它本身不自动证明吸收更好，吸收相关表述需要独立证据支持。

## Usage Context
- product identity
- dosage form
- material terminology
- translation

## Approved Usage Examples
### English
- "Dosage form: softgel."

### 中文
- "剂型：软胶囊。"

## Discouraged Usage Examples
### English
- "Softgel guarantees better absorption."

### 中文
- "软胶囊一定吸收更好。"

## Notes
- literal translation: 软胶囊
- business rewrite: softgel dosage form
- status note: Current for terminology normalization only.
- review behavior: Human review is not required for the basic term mapping; product-specific form claims and absorption claims remain source-linked and review-sensitive.
- risk notes: Low terminology risk; claims attached to softgel remain review-sensitive.

## Source Map
- [[SRC-WIDE-DECK-EN]]: WIDE NATURALS DECK-ENpptx.pptx
- [[SRC-PG-UBIQUINOL-CN-CARD]]: PG加拿大品牌第四代共晶还原性辅酶Q10头部手卡.pptx
